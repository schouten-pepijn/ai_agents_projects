...
class Config:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(
            self,
            "llm",
            ChatOllama(model=self.model, base_url=self.base_url, temperature=0),
        )
...
REPAIR_PROMPT = ChatPromptTemplate(
    [
        ...
        (
            "human",
            """The previous code failed or was inadequate.
...
EVAL_PROMPT = ChatPromptTemplate(
    [
        (
            "system",
            """You are a strict code evaluator. Output a terse JSON object with keys:
...
def run_in_subprocess(module_code: str, timeout_sec: int = 15) -> ExecResult:
    ...
    cmd = [
        sys.executable,
        "-c",
        "import runpy; m=runpy.run_path('solution.py'); m['run']()",
    ]
    ...
    try:
        ...
        return ExecResult(
            returncode=p.returncode, stdout=p.stdout, stderr=p.stderr, path=path
        )

    except subprocess.TimeoutExpired as e:
        return ExecResult(
            returncode=124, stdout=e.stdout or "", stderr="TimeoutExpired", path=path
        )

    except Exception as e:
        return ExecResult(returncode=1, stdout="", stderr=str(e), path=path)
...
def draft_code(state: AgentState) -> AgentState:
    msg = GEN_PROMPT.invoke({"request": state["user_request"]})
...
def exec_code(state: AgentState) -> AgentState:
    ...
    if violation:
        res = ExecResult(
            returncode=2, stdout="", stderr=f"Safety check failed: {violation}", path=""
        )
    else:
...
def evaluate(state: AgentState) -> AgentState:
    ...
    return {**state, "eval_report": json.dumps(concise, ensure_ascii=False)}
...
def decide(state: AgentState) -> AgentState:
    ...
    if ok or state["attempt"] >= state["max_attempts"]:
        return {**state, "decision": "final"}

    return {**state, "decision": "repair"}
...
def repair(state: AgentState) -> AgentState:
    ...
    return {**state, "code": new_code, "attempt": state["attempt"] + 1}
...
graph.add_conditional_edges(
    "decide", lambda s: s["decision"], {"final": END, "repair": "repair"}
)
...
with gr.Blocks(title="Python Coding Assistant") as demo:
    ...
    with gr.Row():
        req = gr.Textbox(
            label="Your request",
            lines=5,
            placeholder="Example: Generate a Python program that creates ASCII art from an image.",
        )
